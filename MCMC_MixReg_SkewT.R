#' Bayesian finite mixture of regression models - Skew-Student-T
#'
#' This function estimates a finite mixture of regression models, 
#' where each component follows a skew-student-t distribution.
#'
...
## MCMC
mixreg_skewt <- function(y,x,numcomp,n_mcmc,burnin_mcmc,thin_mcmc){
  yx<-cbind(y,x)
  yx<-yx[order(yx[,1]),]
  y<-yx[,1]
  x<-yx[,2:(1+dim(x)[2])]
  ...
  for (i in 1:numcomp){
    ...
    uaux    <- c(uaux,rgamma(length(x_comp[,1]),nu[1,i]/2,nu[1,i]/2))
  }
  v1 <- data.frame(x1=y)
...
atualizarS<-function(y,p,mu,x,beta,sigma2,lambda,nu,N,numcomp,numcov){
  ...
  for(g in 1:numcomp){
    ...
    matrizaux[g,]<-p[g]*dst(y,escala,sqrt(sigma2[g]),lambda[g],nu=nu[g])
  }
...
atualizarU<-function(nu,y,x,beta,mu,psi,tau,z,N){
  ...
  beta<-nu/2+(y-mu-x%*%beta-psi*z)^2/(2*tau^2)+0.5*z^2
